...
# reduced model
droppedDV <- rma.mv(ES_Up_Down, V, mods = ~ moderator, random = ~ 1 | Study_Code/Order.for.CMA, data=reaction, knha=TRUE)
droppedDV
anova(moderator, btt=2)
anova(moderator, btt=3:6)

par(mar=c(1,1,1,1))
par(mfrow=c(2,1))
profile(droppedDV, sigma2=1)
profile(droppedDV, sigma2=2)

########### AND THE REST WAS NOT NEEDED ######################
